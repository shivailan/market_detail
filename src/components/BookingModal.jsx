...
export default function BookingModal({ pro, session, onClose, dark, setView }) {
  ...
  const disabledDays = (date) => {
    ...
    // On met en majuscule la première lettre car Supabase stocke souvent "Lundi"
    const formattedDay = dayName.charAt(0).toUpperCase() + dayName.slice(1);
    
    // Si le jour n'est pas dans la liste working_days du pro, on le désactive
    return !pro.working_days?.includes(formattedDay);
  };

  // 2. Générer les créneaux horaires dynamiquement selon pro.working_hours
  const generateAvailableSlots = () => {
    ...
    // On crée un créneau toutes les heures
    ...
    <div className="space-y-4">
      ...
      {pro.services_list && pro.services_list.length > 0 ? (
        pro.services_list.map((s, i) => (
          ...
          >
            <div className="max-w-[70%]">
              <p className="text-lg group-hover:text-[#bc13fe] transition-colors uppercase font-black italic">{s.titre || 'Service sans nom'}</p>
              {/* On affiche un petit bout de la description pour aider le client */}
              <p className="text-[9px] opacity-40 font-medium normal-case line-clamp-1 italic">
                {s.desc || "Consulter les détails techniques sur le profil."}
              </p>
            </div>
        ...
        <div className="text-center py-10 opacity-30 italic">
          <p>Aucun protocole technique configuré par ce studio.</p>
        </div>
      )}
    </div>
  </div>
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 flex flex-col items-center">
              ...
              <div className={`p-6 rounded-[40px] border border-current/10 mb-10 w-full flex justify-center ${dark ? 'bg-white/[0.02]' : 'bg-black/[0.02]'}`}>
                <style>{`
                  .rdp-day_selected { background-color: #bc13fe !important; color: white !important; font-weight: 900 !important; border-radius: 12px !important; }
                  .rdp-button:hover:not([disabled]):not(.rdp-day_selected) { background-color: #00f2ff !important; color: black !important; border-radius: 12px !important; }
                  .rdp-day_disabled { opacity: 0.1 !important; pointer-events: none !important; }
                `}</style>
                <DayPicker
                  mode="single"
                  ...
                  locale={fr}
                  disabled={disabledDays} // <--- FILTRE DES JOURS ICI
                  className="font-black italic uppercase"
                />
              </div>

              <div className="w-full">
                <p className="text-[10px] opacity-30 mb-6 tracking-widest text-center">Créneaux_Ouverts_Studio</p>
                <div className="grid grid-cols-4 gap-3">
                  {dynamicSlots.length > 0 ? dynamicSlots.map(t => (
                    <button 
                      key={t}
                    ...
                    <p className="col-span-4 text-[9px] text-red-500 text-center">Aucun créneau configuré.</p>
                  )}
                </div>
              </div>
            </div>
          ...
          {step === 3 && (
            <div className="animate-in zoom-in-95 text-center pb-10">
              <div className="w-24 h-24 bg-[#bc13fe]/10 text-[#bc13fe] rounded-full flex items-center justify-center mx-auto mb-10 text-4xl shadow-[0_0_40px_rgba(188,19,254,0.2)]">
                <i className="fas fa-shield-alt"></i>
              </div>
              <h3 className="text-4xl mb-4 tracking-tighter leading-none italic font-black uppercase">Validation_Protocole</h3>
              <p className="text-[10px] opacity-40 mb-12">Transmission de la requête au studio</p>
              ...
              <div className={`p-10 rounded-[50px] text-left mb-12 border border-current/10 ${dark ? 'bg-white/[0.03]' : 'bg-black/[0.03]'}`}>
                <div className="flex justify-between border-b border-current/5 pb-5 mb-5 text-sm">
                    <span className="opacity-30 italic font-black uppercase">Unité</span>
                    <span className="italic font-black uppercase">{pro.nom_commercial}</span>
                </div>
                <div className="flex justify-between border-b border-current/5 pb-5 mb-5 text-sm">
                    <span className="opacity-30 italic font-black uppercase">Service</span>
                    <span className="italic font-black uppercase">{selectedService.service}</span>
                </div>
                <div className="flex justify-between border-b border-current/5 pb-5 mb-5 text-sm">
                    <span className="opacity-30 italic font-black uppercase">Timeline</span>
                    <span className="text-[#00f2ff] italic font-black uppercase">{format(selectedDate, 'dd MMMM yyyy', { locale: fr })} @ {selectedTime}</span>
                </div>
                <div className="flex justify-between text-3xl pt-5 font-black">
                    <span className="tracking-tighter italic uppercase">Total</span>
                    <span className="text-[#bc13fe] italic font-black uppercase">{selectedService.price}€</span>
                </div>
              </div>
              ...
              >
                {loading ? 'INITIALISATION...' : 'CONFIRMER LA MISSION'}
              </button>
            </div>
          )}
        </div>
